...
model = YOLO(r'D:\PHICURE\atm\best.pt')
...
phone_model = YOLO(r'D:\PHICURE\atm\Phone_Recognition\Phone_Recognition\best.pt', verbose=False)
...
def video_stream(): 
    ...
    while True:
        ...
        while cap.isOpened():
          ...
          if confirmed:
            ...
            if time.time() - start_time >= 18:
                ...
                if total_count==1:
                    warning_path = os.path.abspath("D:\\PHICURE\\atm\\UIUX\\html\\warning.html")
                    webbrowser.open(f"file://{warning_path}")
                    time.sleep(7)  # 대기 시간

                if total_count == 2:
                   warning_path = os.path.abspath("D:\\PHICURE\\atm\\UIUX\\html\\warning2.html")
                   webbrowser.open(f"file://{warning_path}")
                   time.sleep(10)  # 대기 시간
                   ...
                   html_path = os.path.abspath("D:\\PHICURE\\atm\\UIUX\\html\\social_login.html")
        ...
        if face_confirmed:
           ...
           while True:
                ...
                if time.time() - start_time > extract_duration:
                        if negative_instance_count > positive_instance_count:
                            print("표정인식 문제 발생")
                            total_count += 1
                            warning_path = os.path.abspath("C:\\Users\\LEEHoyJoung\\project\\atm\\UIUX\\html\\warning3.html")
                            webbrowser.open(f"file://{warning_path}")
                            time.sleep(5)  
                        break
            ...
            if danger_time >= 3:
                print("주위살피는 행동입니다")
                warning_path = os.path.abspath("D:\\PHICURE\\atm\\UIUX\\html\\warning_look.html")
                webbrowser.open(f"file://{warning_path}")
                time.sleep(5)  # 대기 시간
            else:
           ...
           print("휴대폰 객체 감지")
           phone_model = YOLO(r'C:\Users\LEEHoyJoung\project\atm\Phone_Recognition\Phone_Recognition\best.pt', verbose=False)
           start_time = time.time()
           ...
           if phone_detected == True:
                print("휴대폰 객체 감지")
                warning_path = os.path.abspath("D:\\PHICURE\\atm\\UIUX\\html\\warning4.html")
                webbrowser.open(f"file://{warning_path}")
                total_count+=1
                time.sleep(5)
...
@app.get("/")
async def read_root():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\videoCam.html")

@app.get("/uiux")
async def read_uiux():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\uiux.html")

@app.get("/uiux2")
async def read_uiux2():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\uiux2.html")

@app.get("/send")
async def read_send():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\send.html")

@app.get("/tongjang")
async def read_tongjang():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\tongjang.html")

@app.get("/withdraw")
async def read_withdraw():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\withdraw.html")

@app.get("/card")
async def read_card():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\card.html")

@app.get("/fin")
async def read_fin():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\fin.html")

@app.get("/start_uiux")
async def read_start_uiux():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\start_uiux.html")

@app.get("/account_send")
async def read_account_send():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\account_send.html")

@app.get("/account_send2")
async def read_account_send2():
    return FileResponse("D:\\PHICURE\\atm\\UIUX\\html\\account_send2.html")
...
async def serve_html(file_name: str):
    base_path = "D:\\PHICURE\\atm\\UIUX\\html\\"
